...
public class UserController {
    ...
    @Operation(
            summary = "회원 목록 조회",
            description = "전체 회원 목록을 조회하거나, 이메일로 특정 회원을 검색합니다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200",
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "404",
                    description = "이메일로 조회 시 회원을 찾을 수 없음"
            )
    })
    public ResponseEntity<ApiResponse<?>> getUsers(
            @Parameter(description = "검색할 이메일 (선택)", example = "user@example.com")
    ...
    @Operation(
            summary = "회원 정보 수정",
            description = "회원의 비밀번호 또는 이름을 수정합니다. 수정하지 않을 필드는 null로 전달하면 됩니다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200",
    ...
    @Operation(
            summary = "회원 탈퇴",
            description = "회원을 삭제합니다. (Soft Delete - 실제로는 삭제되지 않고 삭제 시간만 기록됩니다)"
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "204",
