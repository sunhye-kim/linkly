...
class CategoryRepositoryTest {
	...
	void saveAndFindCategory() {
		// given
		Category category = Category.builder().appUser(testUser).name("개발").description("개발 관련 북마크").build();
	...
	void findByAppUserAndName() {
		// given
		Category category = Category.builder().appUser(testUser).name("디자인").description("디자인 자료").build();
	...
	void existsByAppUserAndName() {
		// given
		Category category = Category.builder().appUser(testUser).name("프로그래밍").build();
		categoryRepository.save(category);
		entityManager.flush();
	...
	void findAllByAppUserAndDeletedAtIsNull() {
		// given
		Category category1 = Category.builder().appUser(testUser).name("카테고리1").build();

		Category category2 = Category.builder().appUser(testUser).name("카테고리2").build();

		Category deletedCategory = Category.builder().appUser(testUser).name("삭제된카테고리").build();
		...
		assertThat(categories).extracting("name").containsExactlyInAnyOrder("카테고리1", "카테고리2");
	}
	...
	void findAllByAppUser_IdAndDeletedAtIsNull() {
		// given
		Category category = Category.builder().appUser(testUser).name("테스트").build();
		categoryRepository.save(category);
		entityManager.flush();
	...
	void differentUsersSameCategory() {
		...
		Category category2 = Category.builder().appUser(anotherUser).name("개발").build();
	...
	void updateCategory() {
		// given
		Category category = Category.builder().appUser(testUser).name("수정 전").description("이전 설명").build();
		categoryRepository.save(category);
		entityManager.flush();
		entityManager.clear();
	...
	void softDelete() {
		// given
		Category category = Category.builder().appUser(testUser).name("삭제 테스트").build();
		categoryRepository.save(category);
		entityManager.flush();
