...
class CategoryControllerTest {
	...
	void getCategoryById_Success() throws Exception {
		// given
		Long categoryId = 1L;
		CategoryResponse response = CategoryResponse.builder().id(categoryId).userId(1L).name("개발")
				.createdAt(LocalDateTime.now()).updatedAt(LocalDateTime.now()).build();
	...
	void getCategoriesByUserId() throws Exception {
		// given
		Long userId = 1L;
		CategoryResponse category1 = CategoryResponse.builder().id(1L).userId(userId).name("개발")
				.createdAt(LocalDateTime.now()).updatedAt(LocalDateTime.now()).build();

		CategoryResponse category2 = CategoryResponse.builder().id(2L).userId(userId).name("디자인")
				.createdAt(LocalDateTime.now()).updatedAt(LocalDateTime.now()).build();
	...
	void updateCategory_Success() throws Exception {
		...
		Long categoryId = 1L;
		Long userId = 1L;
		UpdateCategoryRequest request = UpdateCategoryRequest.builder().name("프로그래밍").description("프로그래밍 관련").build();

		CategoryResponse response = CategoryResponse.builder().id(categoryId).userId(userId).name("프로그래밍")
				.description("프로그래밍 관련").createdAt(LocalDateTime.now()).updatedAt(LocalDateTime.now()).build();
