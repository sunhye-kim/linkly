...
/**
 * Repository 계층 테스트 Django의 Model 테스트와 유사 @DataJpaTest: JPA 관련 컴포넌트만 로드 (빠른
 * 테스트) - Entity, Repository만 로드 - 각 테스트 후 자동 롤백 - 인메모리 DB 사용
 */
@DataJpaTest
...
class BookmarkRepositoryTest {
	...
	void setUp() {
		// 테스트용 사용자 생성
		testUser = AppUser.builder().email("test@example.com").password("password123").name("테스트 사용자").build();
		testUser = userRepository.save(testUser);

		// 테스트용 카테고리 생성
		testCategory = Category.builder().appUser(testUser).name("개발").description("개발 관련 북마크").build();
	...
	void saveAndFindBookmark() {
		// given
		Bookmark bookmark = Bookmark.builder().appUser(testUser).category(testCategory).url("https://example.com")
				.title("테스트 북마크").description("테스트용 북마크입니다").build();
	...
	void findAllByAppUser() {
		...
		// 다른 사용자의 북마크
		AppUser otherUser = userRepository
				.save(AppUser.builder().email("other@example.com").password("password123").name("다른 사용자").build());
		Bookmark otherBookmark = Bookmark.builder().appUser(otherUser).url("https://other.com").title("다른 사용자 북마크")
				.build();
		...
		assertThat(bookmarks).extracting("url").containsExactlyInAnyOrder("https://example1.com",
				"https://example2.com");
	}
	...
	void findAllByCategory() {
		...
		// 다른 카테고리
		Category otherCategory = categoryRepository.save(Category.builder().appUser(testUser).name("디자인").build());
		Bookmark otherBookmark = Bookmark.builder().appUser(testUser).category(otherCategory).url("https://other.com")
				.title("다른 카테고리 북마크").build();
		bookmarkRepository.save(otherBookmark);
	...
	void bookmarkTagMapping() {
		...
		Tag tag1 = tagRepository.save(Tag.builder().appUser(testUser).name("Java").build());

		Tag tag2 = tagRepository.save(Tag.builder().appUser(testUser).name("Spring").build());

		BookmarkTagMap mapping1 = bookmarkTagMapRepository
				.save(BookmarkTagMap.builder().bookmark(bookmark).tag(tag1).build());

		BookmarkTagMap mapping2 = bookmarkTagMapRepository
				.save(BookmarkTagMap.builder().bookmark(bookmark).tag(tag2).build());
		...
		assertThat(mappings).extracting(m -> m.getTag().getName()).containsExactlyInAnyOrder("Java", "Spring");
	}
	...
	void findBookmarksByTag() {
		...
		Tag javaTag = tagRepository.save(Tag.builder().appUser(testUser).name("Java").build());

		bookmarkTagMapRepository.save(BookmarkTagMap.builder().bookmark(bookmark1).tag(javaTag).build());

		bookmarkTagMapRepository.save(BookmarkTagMap.builder().bookmark(bookmark2).tag(javaTag).build());
		...
		// then
		assertThat(mappings).hasSize(2);
		assertThat(mappings).extracting(m -> m.getBookmark().getTitle()).containsExactlyInAnyOrder("북마크1", "북마크2");
	}
	...
	void existsByBookmarkAndTag() {
		...
		Tag tag = tagRepository.save(Tag.builder().appUser(testUser).name("Java").build());

		bookmarkTagMapRepository.save(BookmarkTagMap.builder().bookmark(bookmark).tag(tag).build());
	...
	private Bookmark createBookmark(String url, String title) {
		return Bookmark.builder().appUser(testUser).category(testCategory).url(url).title(title).description("테스트 설명")
				.build();
	}
}
